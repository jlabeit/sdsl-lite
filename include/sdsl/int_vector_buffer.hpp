...
void construct_bwt(cache_config& config)
{
    ...
    size_type buffer_size = 1024*1024; // buffer_size is a multiple of 8!, TODO: still true?
    ...
    uint8_t sa_width = sa_buf.width();

    // (3) Calculate buffersize such that all buffers are reloaded at same time
    buffer_size = (size_type)bwt_width * (size_type)sa_width * 1024;
    bwt_buf.buffersize(buffer_size);
    sa_buf.buffersize(buffer_size);
    buffer_size /= (size_type)bwt_width * (size_type)sa_width;
